...
{
    public class PinkyBehavior : BehaviorBase
    {
        const float EyeHeight = 1.50f;
        const float AttackHeight = 1.2f;
        const float ViewFrustumMin = -.15f;
        const float SeeDistance = 400f;
        const float IdleChance = 0f;
        ...
        const float meleeAttackRange = 2.5f;

        const int MeleeDamageMin = 4;
        const int MeleeDamageMax = 40;
        ...
        {
            if (damageType == DamageType.ImpFire)
                amount = 0;
        }
        ...
        public override void Init()
        {
            ...
            if (mc == null)
                UnityEngine.Debug.LogError("PinkyBehavior: Init: Could not get component \"MonsterController\" from owner \"" + owner.thingName + "\"");
        ...
        bool CanMeleeRay(float distance)
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(ray, out hit, distance, ~((1 << 9) | (1 << 10) | (1 << 11) | (1 << 12)), QueryTriggerInteraction.Ignore))
                return false;
