...
namespace DoomLoader
{
    public class WadLoader : MonoBehaviour 
    {
        ...
        public static bool LoadWad(string file)
        {
            ...
            if (!File.Exists(path))
            {
                UnityEngine.Debug.LogError("WadLoader: Load: File \"" + file + "\" does not exist!");
                return false;
            }
            ...
            if (file.Length < 4)
            {
                reader.Close();
                stream.Close();
                UnityEngine.Debug.LogError("WadLoader: Load: WAD length < 4!");
                return false;
            }
            ...
            try
            {
                ...
                UnityEngine.Debug.Log("Loaded WAD \"" + file + "\" (" + bytes + " bytes in lumps)");
            }
            catch(Exception e)
            {
                UnityEngine.Debug.LogError("WadLoader: Load: Reader exception!");
